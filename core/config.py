...
class TableauServerConfig:
    """Tableau Server 配置"""
    server_url: str = "http://win-picvs30bivi"
    default_project: str = "数据源"
    project_luid: str = "7fa01ebc-72f4-4fd0-a27f-f8ac1e30ac20"
...
def _find_project_root() -> Path:
    ...
    current = Path(__file__).parent.parent  # core/ 的上级目录
    return current
...
def load_config(
    ...
    server_config = TableauServerConfig(
        server_url=server_yaml.get("url", "http://win-picvs30bivi"),
        default_project=server_yaml.get("default_project", "数据源"),
...
try:
    ...
    DEFAULT_CONFIG = TFLConfig(
        ...
        database=DatabaseConfig(
            host="rm-uf661w842h18w326nvo.mysql.rds.aliyuncs.com",
            port="3306",
            username="link_BI",
            dbname="voxadmin",
            db_class="mysql"
